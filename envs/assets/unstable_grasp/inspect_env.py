...
force = 0.22
sim.update_contact_parameters('load', 'box', mu=0.11, kn=5e3, kt=1e2, damping=1e2)
...
ax1.plot(np.linspace(0, ts.shape[0] * 0.005, ts.shape[0] - ts_offset),
         np.linalg.norm(ts, axis=-1).reshape(nstep, -1).mean(axis=1)[ts_offset:] / 0.001, color='b')
...
ax2.plot(np.linspace(0, ts.shape[0] * 0.005, ts.shape[0] - ts_offset),
         qs[ts_offset:, 12], color='g')
