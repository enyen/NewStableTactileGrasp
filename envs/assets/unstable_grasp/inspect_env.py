...
sim.update_contact_parameters('weight', 'box', mu=0.15, kn=5e3, kt=1e2, damping=1e2)
sim.update_body_density('weight', 5000)
sim.update_body_size('weight', (0.025, 0.02, 0.02))
...
# down
for t in range(50):
    # u = qpos_end[:5] + t / 50. * (qpos_init[:5] - qpos_end[:5])
